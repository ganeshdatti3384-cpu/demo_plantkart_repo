...
export default function VendorAccommodationBookings() {
  ...
  const fetchBookings = () => {
    setLoading(true);
    fetch('/api/vendor/accommodation/bookings')
      .then(res => res.json())
      .then(data => {
        setBookings(data);
        setLoading(false);
  ...
  const handleStatusChange = async (bookingId: string, status: string) => {
    await fetch('/api/vendor/accommodation/bookings/status', {
      method: 'POST',
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-slate-50 dark:bg-slate-950">
      <Sidebar activeService="Property Bookings" />
