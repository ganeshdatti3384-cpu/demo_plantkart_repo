...
export default function CarRentalPage() {
  ...
  }, [searchLocation, typeFilter, cars]);

  const handleAction = async (car: any) => {
    setIsProcessing(true);
    try {
      // Create a temporary car request
      ...
      if (data.success) {
        toast.success(`Initiating secure ${car.type === 'SELL' ? 'purchase' : 'rental'}...`);
        ...
        const paymentRes = await fetch('/api/payments/checkout', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            amount: car.price.replace(/[^0-9.]/g, ''),
            name: `${car.type === 'SELL' ? 'Buy' : 'Rent'} ${car.title}`,
            requestId: data.id,
            type: 'CAR'
          })
        });
            ...
            <>
              ...
              {filteredCars.length === 0 ? (
                <div className="py-20 text-center">
                  <p className="text-slate-500 font-bold mb-4">No vehicles found matching your criteria.</p>
                  <button 
                    onClick={() => { setSearchLocation(''); setTypeFilter('all'); }}
                ...
                <div className="flex flex-col md:flex-row h-full max-h-[95vh] md:max-h-[90vh]">
                  ...
                  <div className="md:w-1/2 p-4 sm:p-10 overflow-y-auto">
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800 pt-4 sm:pt-8 mt-auto">
                      ...
                      >
                        {isProcessing ? 'Processing...' : selectedCar.type === 'SELL' ? 'Purchase Now' : 'Rent This Car'}
                      </button>
                      <p className="text-center text-[8.5px] sm:text-[10px] text-slate-400 mt-3 uppercase tracking-widest">Transactions secured by Stripe</p>
                    </div>
                  </div>
                </div>
              </div>
