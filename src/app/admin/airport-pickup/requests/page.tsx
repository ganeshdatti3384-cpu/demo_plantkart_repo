...
export default function AdminPickupRequests() {
  ...
  const [modalType, setModalType] = useState<'accept' | 'details' | 'driver' | 'approve'>('accept');
  ...
  const [pickupPersonData, setPickupPersonData] = useState({
    pickupPersonName: '',
    pickupPersonPhone: '',
    relationship: '',
    meetingInstructions: ''
  ...
  const updateStatus = async (id: string, action: string, body = {}) => {
    const endpoint = action.toLowerCase() === 'approve' 
      ? 'approve'
      : action.toLowerCase();
    
    const res = await fetch(`/api/admin/airport-pickup/${id}/${endpoint}`, {
      method: 'PUT',
  ...
  const handleAction = (req: any, type: 'accept' | 'details' | 'driver' | 'approve') => {
    ...
    if (type === 'approve') {
      setPickupPersonData({
        pickupPersonName: req.pickupPersonName || '',
        pickupPersonPhone: req.pickupPersonPhone || '',
        relationship: req.pickupPersonRelationship || '',
                ...
                <tr key={req._id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter ${
                      ...
                      req.status === 'APPROVED' ? 'bg-purple-500/20 text-purple-500' :
                      'bg-amber-500/20 text-amber-500'
                    }`}>
                      {req.status === 'COMPLETED' && <FaCheckCircle className="size-2.5" />}
                      {req.status === 'APPROVED' && <FaUserCheck className="size-2.5" />}
                      {req.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {/* Approve Button - Show for PENDING status */}
                      {req.status === 'PENDING' && (
                        <button 
                          onClick={() => handleAction(req, 'approve')}
                          className="p-2 bg-purple-600/10 hover:bg-purple-600/20 text-purple-400 rounded-lg transition-colors"
                          title="Approve Request"
                        >
                          <FaUserCheck className="size-3.5" />
                        </button>
                      )}
                      
                      {/* Assign Driver Button - Show for APPROVED status */}
                      ...
                      <button 
                        ...
                        className="p-2 bg-green-600/10 hover:bg-green-600/20 text-green-400 rounded-lg transition-colors"
                        title="View Details"
                      >
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[3rem] p-8 md:p-12 shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            {modalType === 'details' ? (
              <div className="space-y-10">
                ...
                {/* Display Pickup Person Info if exists */}
                {selectedRequest.pickupPersonName && (
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-purple-600 uppercase tracking-[0.2em] flex items-center gap-2">
                      <FaUserCheck /> Pickup Person Details
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-purple-50/30 dark:bg-purple-900/10 p-6 rounded-3xl border border-purple-100 dark:border-purple-900/30">
                      <div>
                        <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest">Name</p>
                        <p className="font-bold text-slate-900 dark:text-white">{selectedRequest.pickupPersonName}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest">Phone</p>
                        <p className="font-bold text-slate-900 dark:text-white">{selectedRequest.pickupPersonPhone}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest">Relationship</p>
                        <p className="font-bold text-slate-900 dark:text-white">{selectedRequest.pickupPersonRelationship || 'N/A'}</p>
                      </div>
                      <div className="md:col-span-2">
                        <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest">Meeting Instructions</p>
                        <p className="font-bold text-slate-900 dark:text-white">{selectedRequest.meetingInstructions || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-medium">Enter pickup person details to approve this request.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Pickup Person Name *</label>
                    <div className="relative">
                      <FaUserCheck className="absolute left-5 top-1/2 -translate-y-1/2 text-purple-500" />
                      <input 
                        type="text" 
                        placeholder="John Doe" 
                        className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl focus:ring-2 focus:ring-purple-500 transition-all font-bold dark:text-white" 
                        value={pickupPersonData.pickupPersonName} 
                        onChange={e => setPickupPersonData({...pickupPersonData, pickupPersonName: e.target.value})} 
                        required
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Pickup Person Phone *</label>
                    <div className="relative">
                      <FaPhone className="absolute left-5 top-1/2 -translate-y-1/2 text-purple-500" />
                      <input 
                        type="text" 
                        placeholder="+61 4XX XXX XXX" 
                        className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl focus:ring-2 focus:ring-purple-500 transition-all font-bold dark:text-white" 
                        value={pickupPersonData.pickupPersonPhone} 
                        onChange={e => setPickupPersonData({...pickupPersonData, pickupPersonPhone: e.target.value})} 
                        required
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Relationship to Passenger</label>
                    <div className="relative">
                      <FaUserAlt className="absolute left-5 top-1/2 -translate-y-1/2 text-purple-500" />
                      <input 
                        type="text" 
                        placeholder="Friend, Family, Colleague, etc." 
                        className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl focus:ring-2 focus:ring-purple-500 transition-all font-bold dark:text-white" 
                        value={pickupPersonData.relationship} 
                        onChange={e => setPickupPersonData({...pickupPersonData, relationship: e.target.value})}
                      />
                    </div>
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Meeting Instructions</label>
                    <textarea 
                      placeholder="Specific meeting point instructions..." 
                      className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-none rounded-2xl focus:ring-2 focus:ring-purple-500 transition-all font-bold dark:text-white resize-none" 
                      rows={2} 
                      value={pickupPersonData.meetingInstructions} 
                      onChange={e => setPickupPersonData({...pickupPersonData, meetingInstructions: e.target.value})}
                    ></textarea>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-1 py-5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-3xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all"
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={() => {
                      if (!pickupPersonData.pickupPersonName || !pickupPersonData.pickupPersonPhone) {
                        toast.error('Please fill in all required fields');
                        return;
                      }
                      updateStatus(selectedRequest._id, 'APPROVE', pickupPersonData);
                    }}
                    className="flex-1 py-5 bg-purple-600 hover:bg-purple-700 text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl shadow-purple-500/20 transition-all"
                  >
                    Approve & Notify User
                  </button>
                </div>
