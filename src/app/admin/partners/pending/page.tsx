...
import { FaGift, FaCheck, FaTimes, FaMapMarkerAlt, FaPercent } from 'react-icons/fa';
    ...
    <div className="min-h-screen">
      <header className="mb-8 lg:flex items-center justify-between gap-8 bg-slate-900/40 border border-white/5 p-8 rounded-[2.5rem] text-white">
        <div>
          <h1 className="text-2xl font-black text-white mb-1 flex items-center gap-3 tracking-tight">
            <div className="w-10 h-10 bg-pink-600 rounded-xl flex items-center justify-center text-white text-lg shadow-lg shadow-pink-500/20">
              <FaGift />
            </div>
            Partner Perks Moderation
          </h1>
          <p className="text-slate-400 text-sm font-bold italic">Review and approve partner discount offers.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative">
            <input 
              ...
              className="bg-slate-800/50 border border-white/10 rounded-2xl px-6 py-2 text-xs font-bold text-white focus:outline-none focus:ring-2 focus:ring-pink-500/50 w-64"
            />
          </div>
        </div>
      </header>
      ...
      {loading ? (
        <div className="p-20 text-center">
          ...
          <p className="text-slate-500 font-black uppercase tracking-widest text-xs">Fetching Partners...</p>
        </div>
      ) : filteredItems.length === 0 ? (
        <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-20 text-center">
          <p className="text-slate-500 font-black uppercase tracking-[0.2em] text-[10px]">No partners matching your search.</p>
        </div>
      ) : (
            ...
            <div key={item._id} className="bg-slate-900/40 border border-white/5 p-6 rounded-[2rem] flex flex-col md:flex-row gap-6 items-center hover:bg-white/5 transition-all group">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 text-[10px] font-bold text-slate-500">
                  ...
                  <span className="flex items-center gap-1.5 font-black text-pink-400 uppercase tracking-tighter"><FaPercent /> {item.discount} OFF</span>
                  <span className="flex items-center gap-1.5 text-slate-400 italic">By: {item.vendorEmail}</span>
                  <span className="flex items-center gap-1.5 text-slate-400">Category: {item.category}</span>
                </div>
              </div>
              {item.status === 'PENDING' && (
