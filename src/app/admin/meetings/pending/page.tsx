...
import { FaUserTie, FaCheck, FaTimes, FaMapMarkerAlt, FaDollarSign, FaClock } from 'react-icons/fa';
import { toast } from 'react-hot-toast';

export default function AdminPendingMeetings() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const res = await fetch('/api/admin/meetings/pending');
  ...
  const handleAction = async (id: string, action: string) => {
    ...
    if (res.ok) {
      toast.success(`Meeting service ${action}ed`);
      fetchItems();
    }
  };
    ...
    <div className="min-h-screen">
      <header className="mb-8 lg:flex items-center justify-between gap-8 bg-slate-900/40 border border-white/5 p-8 rounded-[2.5rem] text-white">
        <div>
          ...
          <p className="text-slate-400 text-sm font-bold italic">Review and approve consultancy meeting services.</p>
        </div>
      ...
      {loading ? (
        <div className="p-20 text-center">
          ...
          <p className="text-slate-500 font-black uppercase tracking-widest text-xs">Fetching Services...</p>
        </div>
      ) : filteredItems.length === 0 ? (
        <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-20 text-center">
          <p className="text-slate-500 font-black uppercase tracking-[0.2em] text-[10px]">No services matching your search.</p>
        </div>
      ) : (
            ...
            <div key={item._id} className="bg-slate-900/40 border border-white/5 p-6 rounded-[2rem] flex flex-col md:flex-row gap-6 items-center hover:bg-white/5 transition-all group">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 text-[10px] font-bold text-slate-500">
                  ...
                  <span className="flex items-center gap-1.5 font-black text-indigo-400 uppercase tracking-tighter"><FaDollarSign /> ${item.price}</span>
                  <span className="flex items-center gap-1.5"><FaClock className="text-slate-400" /> {item.duration} min</span>
                  <span className="flex items-center gap-1.5 text-slate-400 italic">By: {item.vendorEmail}</span>
                  <span className="flex items-center gap-1.5 text-slate-400">Specialization: {item.specialization}</span>
                </div>
              </div>
              {item.status === 'PENDING' && (
